...
exports.generateCareerReport = exports.generateReportLogic = void 0;
// Placeholder for report generation logic
const generateReportLogic = (birthInfo, chartData) => {
    // Implement report generation
    return { content: 'placeholder report' };
};
exports.generateReportLogic = generateReportLogic;
...
exports.generateCareerReport = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
    ...
    if (cacheDoc.exists) {
        const cachedReport = cacheDoc.data();
        // 检查快取中的 promptVersion 是否与当前代码版本匹配
        if (((_a = cachedReport.metadata) === null || _a === void 0 ? void 0 : _a.promptVersion) === promptVersion) {
            console.log(`快取命中 (Cache Hit): ${cacheId}, 版本: ${promptVersion}`);
            // 快取命中，标记来源为 "cache" 并直接返回
            return Object.assign(Object.assign({}, cachedReport), { metadata: Object.assign(Object.assign({}, cachedReport.metadata), { source: "cache" }) });
        }
        console.log(`快取失效 (Stale Cache): ${cacheId}, 旧版本: ${(_b = cachedReport.metadata) === null || _b === void 0 ? void 0 : _b.promptVersion}, 新版本: ${promptVersion}`);
    }
    else {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!userDoc.exists) {
            throw new functions.https.HttpsError("not-found", "找不到用户资料。");
        }
    ...
    // --- 配额检查通过，继续执行报告生成逻辑 ---
    // 4. 调用 Gemini 客户端生成报告
    const validatedReport = await (0, geminiClient_1.generateStructuredCareerReport)(baziData);
    // 5. 封装元数据 (Metadata)
