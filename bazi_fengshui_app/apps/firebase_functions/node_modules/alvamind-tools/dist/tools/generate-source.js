...
async function generateSourceCodeMarkdown(options) {
    ...
    console.log(chalk.green(`✓ Found ${chalk.bold(matchingFiles.length)} files in ${chalk.bold(directories.length)} dirs\n`));
    ...
    for (const file of matchingFiles) {
        process.stdout.write(`\r${chalk.dim(`Processing: ${processedFiles}/${matchingFiles.length} files`)}`);
        output += `// ${file}\n`;
    ...
    try {
        ...
        console.log(chalk.dim(`• Blank lines: ${chalk.cyan(removeBlankLines ? 'Removed' : 'Preserved')}`));
...
function parseArgs(args) {
    ...
    try {
        ...
        console.log(chalk.dim('generate-source --include=main.test.ts,test.interface.ts --exclude=dist/,build/ --output=docs.md\n'));
        process.exit(1);
    }
    return options;
}
...
if (args.length === 0 || args.includes('--help')) {
    ...
    console.log(chalk.dim('  generate-source --include=src/ --preserve-blank-lines --preserve-comments\n'));
    process.exit(0);
}
